...
object VideoResumeStore {
    ...
    fun requestStart(mediaId: String, positionMs: Long) {
        requestedStarts[mediaId] = RequestedStart(positionMs.coerceAtLeast(0L), SystemClock.elapsedRealtime())
    }
    ...
    fun load(context: Context, mediaId: String): Long = prefs(context).getLong(mediaId, 0L).coerceAtLeast(0L)
    ...
    fun save(context: Context, mediaId: String, positionMs: Long, durationMs: Long) {
        ...
        prefs(context).edit().apply {
            if (normalized == null) remove(mediaId) else putLong(mediaId, normalized)
        }.apply()
    }
...
internal fun normalizeVideoResumePosition(positionMs: Long, durationMs: Long): Long? {
    ...
    if (durationMs > 0L) {
        ...
        val endGuardMs = minOf(VIDEO_RESUME_MAX_END_GUARD_MS, (durationMs * VIDEO_RESUME_END_GUARD_PERCENT / 100L).coerceAtLeast(1L))
