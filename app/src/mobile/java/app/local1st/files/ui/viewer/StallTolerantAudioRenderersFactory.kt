 ...
 * Workaround for #124.
 *
 * Some AudioSink/AudioTrack clock paths briefly stop reporting progress even though playback keeps
 * moving. When ExoPlayer uses that renderer clock as the master clock, video frame release pauses
 * with it. Keep the audio renderer as the master clock, but bridge only short reporting stalls with
 * elapsed realtime so A/V sync remains anchored to audio in normal playback.
 */
@UnstableApi
internal class StallTolerantAudioRenderersFactory(
    context: Context,
...
private class StallTolerantMediaClock(
    ...
    override fun getPositionUs(): Long {
        ...
        if (delegateDeltaUs > 0L) {
            ...
            // In the normal case the real audio clock remains authoritative. If a short clock stall
            // was bridged and the delegate has not caught up yet, keep output monotonic while giving
            // the delegate time to converge naturally.
            val returned = if (
    ...
    private companion object {
        // 29.97 fps is ~33.4 ms/frame. Start bridging before a two-frame visible pause forms.
        const val STALL_DETECTION_US = 25_000L
        // The observed #124 gap is ~200 ms. Cap extrapolation so a real long stall cannot run away.
        const val MAX_STALL_COMPENSATION_US = 250_000L
        // User seeks / timeline changes should reset the interpolation state immediately.
